...
def run(dataset, resolution, result_dir, DiffAugment, num_gpus, batch_size, total_kimg, ema_kimg, num_samples, gamma, fmap_base, fmap_max, latent_size, mirror_augment, impl, metrics, resume, resume_kimg, num_repeats, eval, show_samples_every):
    ...
    if show_samples_every is not None:
        kwargs.update(image_snapshot_ticks=show_samples_every)
...
def main():
    ...
    parser.add_argument('--show-samples-every', help='Sample and show GAN samples every n ticks', default=1, type=int)
